...
public final class Fraction extends Number implements Comparable<Fraction> {
    ...
    public static Fraction max(Fraction f, Fraction g) {
        if (f.compareTo(g) > 0)
            return f;
        return g;
    }

    public static Fraction min(Fraction f, Fraction g) {
        if (f.compareTo(g) < 0)
            return f;
        return g;
    }
    ...
    public Fraction multiplyBy(final int factor) {
        if (numerator == 0 || factor == 0) {
            return ZERO;
        }
        return Fraction.getFraction(numerator*factor, denominator).reduce();
    }
    ...
    public Fraction divideBy(final int factor) {
        if (factor == 0) {
            throw new ArithmeticException("The fraction to divide by must not be zero");
        }
        return Fraction.getFraction(numerator, denominator*factor).reduce();
    }
